...
func _on_body_entered(body: Node3D) -> void:
	...
	print(body.held_item)
	if body.held_item: Overlay.add_action(Action.BUTTONS.E, "Place item")
	if current_item and $PopUpTexture.process == 0.0: Overlay.add_action(Action.BUTTONS.R, "Start processing")
...
func _input(event: InputEvent) -> void:
	...
	if event.is_action("interact1") and event.is_pressed() and not event.is_echo():
		if get_tree().get_first_node_in_group("Player").held_item:
			...
			print(get_tree().get_first_node_in_group("Player").held_item)
			Overlay.remove_action_by_text("Place item")
			Overlay.add_action(Action.BUTTONS.R, "Start processing")
			...
			current_item = null
			$Popup.end_process()
	if event.is_action("interact3") and event.is_pressed() and not event.is_echo():
